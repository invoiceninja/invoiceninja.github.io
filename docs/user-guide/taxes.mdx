...
You can enable both at the same time, and you can switch between one and three taxes per line or per invoice. The trade-off is a small rounding difference between the two approaches, covered in [the User Guide's Line-Level vs Invoice-Level Tax note](/docs/user-guide/user-guide#line-level-vs-invoice-level-tax). Both are valid — pick whichever your accountant or tax authority expects.
...
Taxes can be configured manually by following the steps [here](/docs/user-guide/basic-settings/#tax_settings), where you define the tax names, rates, and how they're applied. Once set up, manual rates are available in the tax selector on every [invoice](/docs/user-guide/invoices), quote, credit, and [product](/docs/user-guide/products).
...
A useful workflow is to assign a default tax rate at the product level. When you drop a product into an invoice, its tax comes with it, so you're not picking a rate by hand for every line. You can also set tax defaults per client from the client's settings (see [Clients](/docs/user-guide/clients)), which is handy when one client is tax-exempt or sits in a jurisdiction with a different rate.

## US Sales Tax

<VideoPlayer src="/assets/videos/taxes/us_sales_tax/us_sales_tax.mpd" isDash={true} id="us-sales-tax-video" />

From v5.6 onwards Invoice Ninja can calculate US sales tax for you. A short primer first, because US sales tax is unlike VAT or GST in a way that shapes how the rest of this section is configured.
...
If a client has sites in multiple states, attach a [location](/docs/user-guide/clients#locations) to each site — the location's address then drives the tax calculation per document, so a single client can be billed under the correct state-level rules without creating duplicate client records.
...
To handle this, each [product](/docs/user-guide/products) carries a tax category. The calculator reads the category, the seller subregion, and the destination address, and picks the right rate for that combination.
...
If you sell to a tax-exempt client, edit the client record and turn on the Tax Exempt switch — Invoice Ninja will then skip tax on their invoices automatically. See [Clients](/docs/user-guide/clients) for where the switch lives.

![Tax Exempt Client](/assets/images/taxes/tax_exempt_client.png)
...
:::info
If you use our Peppol integration, additional VAT numbers can be stored in Invoice Ninja and the system will pick the right VAT identifier on each [e-invoice](/docs/user-guide/einvoicing) based on the destination.
:::
...
In the EU, tax categories are also configured per [product](/docs/user-guide/products) and drive how the calculator treats each line.

| Tax Category | Description |
|-------------|-------------|
| Physical | A physical product for sale, e.g. a car |
| Service | A service-based product such as bricklaying |
| Digital | A digital product such as a software service |
| Shipping | A charge relating to shipment |
| Exempt | A product that is tax exempt |
| Reduced | A product that attracts a reduced rate |
| Override | A product that doesn't fit a standard category — a custom name and rate are applied |
| Zero Rated | A product that attracts a 0% rate |
| Reverse Tax | Not charged at origin; the buyer pays the tax at their end (reverse-charge) |
| Intra Community | A cross-border supply that carries no tax under intra-community supply rules |
...
A few client fields (see [Clients](/docs/user-guide/clients)) carry real weight once Calculate Taxes is on.
...
Where a valid VAT number is present, the calculator takes it into account and applies or removes tax based on the client's location. The VAT number must pass VIES validation — you can check a number manually [here](https://ec.europa.eu/taxation_customs/vies/).

:::warning
Self-hosted users need the PHP SOAP extension installed for VAT number validation to work.
:::
