...
You can enable both at the same time, and you can switch between one and three taxes per line or per invoice. The trade-off is a small rounding difference between the two approaches, covered in [Get Started's Line-Level vs Invoice-Level Tax note](/docs/user-guide#line-level-vs-invoice-level-tax). Both are valid — pick whichever your accountant or tax authority expects.
